# exportar-ovas-v2.ps1 - Exporta 3 copias OVA de cada VM del Lab 01 (15 OVAs).
# v2: NUNCA exporta la misma VM en paralelo (leccion: 3 exports simultaneos del mismo VDI se traban).
# Uso: powershell -NoProfile -ExecutionPolicy Bypass -File exportar-ovas-v2.ps1
$ErrorActionPreference = 'Continue'
$vbm = 'C:\Program Files\Oracle\VirtualBox\VBoxManage.exe'
$out = 'C:\Users\RANGE\Downloads\maquinas\copias'
$logFile = Join-Path $out 'export-v2.log'
...
Write-Log '=== INICIO exportacion v2 ==='
    ...
    foreach ($id in @($active.Keys)) {
        ...
        if (-not $proc) {
            ...
            if ((Test-Path $j.Ova) -and (Get-Item $j.Ova).Length -gt 100MB) {
                ...
                Write-Log "FAIL  $($j.VM) -> $($j.Ova) (archivo inexistente o <100MB; ver $($j.Err))"
                $failures += $j.Ova
            }
        }
    }
